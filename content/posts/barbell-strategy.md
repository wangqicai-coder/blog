...
杠铃策略的核心是**分离风险**。它是[凸性](/posts/convexity/)思想的直接应用：构建一个下行有限、上行开放的整体结构。
...
这部分本质上是一组[选择权](/posts/optionality/)：有限的下行，无限的上行。
...
杠铃策略是塔勒布"反脆弱"概念的投资应用，也是对[遍历性](/posts/ergodicity/)问题的正确回应。
...
传统分散组合是强健的——试图最小化波动。
杠铃组合是反脆弱的——波动越大，高风险部分的期望值越高（因为[凸性](/posts/convexity/)）。
